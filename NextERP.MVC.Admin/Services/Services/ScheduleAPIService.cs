...
namespace NextERP.MVC.Admin.Services.Services
{
    public class ScheduleAPIService : BaseAPIService, IScheduleAPIService
    {
        ...
        public async Task<APIBaseResult<PagingResult<ScheduleModel>>> GetPaging(FilterModel filter)
        {
            return await PostAsync<APIBaseResult<PagingResult<ScheduleModel>>, FilterModel>($"{Constants.UrlGetSchedules}/{Constants.Filter}", filter);
        }
        ...
        {
            return await ExportAsync<APIBaseResult<byte[]>, FilterModel>(Constants.UrlExportSchedule, filter);
        }
