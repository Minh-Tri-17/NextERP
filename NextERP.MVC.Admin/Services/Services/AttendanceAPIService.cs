...
namespace NextERP.MVC.Admin.Services.Services
{
    public class AttendanceAPIService : BaseAPIService, IAttendanceAPIService
    {
        ...
        {
            return await PostAsync<APIBaseResult<PagingResult<AttendanceModel>>, FilterModel>($"{Constants.UrlGetAttendances}/{Constants.Filter}", filter);
        }
        ...
        {
            return await ExportAsync<APIBaseResult<byte[]>, FilterModel>(Constants.UrlExportAttendance, filter);
        }
