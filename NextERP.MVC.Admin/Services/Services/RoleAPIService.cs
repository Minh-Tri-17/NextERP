...
namespace NextERP.MVC.Admin.Services.Services
{
    public class RoleAPIService : BaseAPIService, IRoleAPIService
    {
        ...
        public async Task<APIBaseResult<PagingResult<RoleModel>>> GetPaging(FilterModel filter)
        {
            return await PostAsync<APIBaseResult<PagingResult<RoleModel>>, FilterModel>($"{Constants.UrlGetRoles}/{Constants.Filter}", filter);
        }
        ...
        {
            return await ExportAsync<APIBaseResult<byte[]>, FilterModel>(Constants.UrlExportRole, filter);
        }
