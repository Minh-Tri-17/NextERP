...
namespace NextERP.MVC.Admin.Services.Services
{
    public class FeedbackAPIService : BaseAPIService, IFeedbackAPIService
    {
        ...
        public async Task<APIBaseResult<PagingResult<FeedbackModel>>> GetPaging(FilterModel filter)
        {
            return await PostAsync<APIBaseResult<PagingResult<FeedbackModel>>, FilterModel>($"{Constants.UrlGetFeedbacks}/{Constants.Filter}", filter);
        }
        ...
        {
            return await ExportAsync<APIBaseResult<byte[]>, FilterModel>(Constants.UrlExportFeedback, filter);
        }
