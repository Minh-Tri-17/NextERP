...
namespace NextERP.MVC.Admin.Services.Services
{
    public class UserAPIService : BaseAPIService, IUserAPIService
    {
        ...
        {
            return await DeleteAsync<APIBaseResult<bool>>($"{Constants.UrlDeleteUser}?ids={ids}");
        }

        public async Task<APIBaseResult<bool>> DeletePermanently(string ids)
        {
            return await DeleteAsync<APIBaseResult<bool>>($"{Constants.UrlDeletePermanentlyUser}?ids={ids}");
        }
        ...
        {
            return await PostAsync<APIBaseResult<PagingResult<UserModel>>, FilterModel>($"{Constants.UrlGetUsers}/{Constants.Filter}", filter);
        }
        ...
        {
            return await ExportAsync<APIBaseResult<byte[]>, FilterModel>(Constants.UrlExportUser, filter);
        }
