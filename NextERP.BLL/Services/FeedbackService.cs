...
namespace NextERP.BLL.Service
{
    public class FeedbackService : IFeedbackService
    {
        ...
        public async Task<APIBaseResult<bool>> CreateOrEdit(Guid id, FeedbackModel request)
        {
            if (id == Guid.Empty)
            {
        ...
        public async Task<APIBaseResult<PagingResult<FeedbackModel>>> GetPaging(Filter filter)
        {
            ...
            var listFeedback = await query
                .OrderByDescending(x => x.DateFeedback ?? x.DateFeedback)
        ...
        public async Task<APIBaseResult<byte[]>> Export(Filter filter)
        {
            ...
            var worksheet = workbook.Worksheets.Add(ObjectNames.Feedback);

            var listFeedback = new List<Feedback>();
            DataHelper.MapAudit(items, listFeedback, string.Empty);
