...
namespace NextERP.BLL.Service
{
    public class AccountService : IAccountService
    {
        ...
        public async Task<APIBaseResult<string>> Auth(UserModel request)
        {
            ...
            var role = _context.Roles.FirstOrDefault(x => x.RoleName == Enums.Role.Admin.ToString().ToLower()
                || x.RoleName == Enums.Role.Developer.ToString().ToLower());

            var roleId = role != null && user.RoleIds != null && user.RoleIds.Contains(role.Id.ToString()) ? role.Id : Guid.Empty;
            ...
            var claims = new Claim[]
            {
                ...
                new Claim(ClaimTypes.Role, DataHelper.GetString(roleId.ToString()))
            };
        ...
        public async Task<APIBaseResult<bool>> Register(UserModel request)
        {
            ...
            var role = await _context.Roles.FirstOrDefaultAsync(x => x.RoleName != null
                && x.RoleName.ToLower() == Enums.Role.Customer.ToString().ToLower());
            if (role == null)
