    ...
    public class TrainingSessionService : ITrainingSessionService
    {
        ...
        public async Task<APIBaseResult<PagingResult<TrainingSessionModel>>> GetPaging(Filter filter)
        {
            ...
            if (!string.IsNullOrEmpty(filter.KeyWord))
            {
                ...
                query = query.Where(x => !string.IsNullOrEmpty(x.SessionCode)
        ...
        public async Task<APIBaseResult<byte[]>> Export(Filter filter)
        {
            ...
            var worksheet = workbook.Worksheets.Add(ObjectNames.TrainingSession);

            var listTrainingSession = new List<TrainingSession>();
            DataHelper.MapAudit(items, listTrainingSession, string.Empty);
