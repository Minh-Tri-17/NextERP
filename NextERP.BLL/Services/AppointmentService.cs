...
namespace NextERP.BLL.Service
{
    public class AppointmentService : IAppointmentService
    {
        ...
        public async Task<APIBaseResult<bool>> CreateOrEdit(Guid id, AppointmentModel request)
        {
            if (id == Guid.Empty)
            {
        ...
        public async Task<APIBaseResult<byte[]>> Export(Filter filter)
        {
            ...
            var worksheet = workbook.Worksheets.Add(ObjectNames.Appointment);

            var listAppointment = new List<Appointment>();
            DataHelper.MapAudit(items, listAppointment, string.Empty);
