...
namespace NextERP.MVC.Services.Services
{
    public class ScheduleAPIService : BaseAPIService, IScheduleAPIService
    {
        ...
        {
            return await PostAsync<APIBaseResult<bool>, ScheduleModel>($"/api/CreateOrEditSchedule", request);
        }

        public async Task<APIBaseResult<bool>> Delete(string ids)
        {
            return await DeleteAsync<APIBaseResult<bool>>($"/api/DeleteSchedule/{ids}");
        }

        public async Task<APIBaseResult<ScheduleModel>> GetOne(Guid id)
        {
            return await GetAsync<APIBaseResult<ScheduleModel>>($"/api/GetSchedule/{id}");
        }

        public async Task<APIBaseResult<PagingResult<ScheduleModel>>> GetPaging(Filter filter)
        {
            return await PostAsync<APIBaseResult<PagingResult<ScheduleModel>>, Filter>($"/api/GetSchedules/Filter", filter);
        }

        public async Task<APIBaseResult<bool>> Import(IFormFile fileImport)
        {
            return await ImportAsync<APIBaseResult<bool>>($"/api/ImportSchedule", fileImport);
        }

        public async Task<APIBaseResult<byte[]>> Export(Filter filter)
        {
            return await ExportAsync<APIBaseResult<byte[]>, Filter>($"/api/ExportSchedule", filter);
        }
    }
}
