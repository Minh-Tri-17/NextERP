...
namespace NextERP.MVC.Services.Services
{
    public class SupplierOrderAPIService : BaseAPIService, ISupplierOrderAPIService
    {
        ...
        {
            return await PostAsync<APIBaseResult<bool>, SupplierOrderModel>($"/api/CreateOrEditSupplierOrder", request);
        }

        public async Task<APIBaseResult<bool>> Delete(string ids)
        {
            return await DeleteAsync<APIBaseResult<bool>>($"/api/DeleteSupplierOrder/{ids}");
        }

        public async Task<APIBaseResult<SupplierOrderModel>> GetOne(Guid id)
        {
            return await GetAsync<APIBaseResult<SupplierOrderModel>>($"/api/GetSupplierOrder/{id}");
        }

        public async Task<APIBaseResult<PagingResult<SupplierOrderModel>>> GetPaging(Filter filter)
        {
            return await PostAsync<APIBaseResult<PagingResult<SupplierOrderModel>>, Filter>($"/api/GetSupplierOrders/Filter", filter);
        }
    }
}
