...
namespace NextERP.MVC.Services.Services
{
    public class BaseAPIService
    {
        ...
        /// <summary>
        /// Tạo một đối tượng HttpClient có kèm theo token xác thực (Bearer Token) lấy từ session.
        /// </summary>
        /// <returns></returns>
        public HttpClient CreateAuthorizedClient()
        {
            // Lấy token từ session hiện tại của người dùng
            var session = _contextAccessor.HttpContext?.Session.GetString(Constants.Token);
            ...
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", session); // Thiết lập Authorization header dạng Bearer Token
