...
namespace NextERP.Util
{
    public static class DataHelper
    {
        ...
        #region Xử lý kiểu dữ liệu data
        ...
        /// <summary>
        /// Dùng để mapping dữ liệu từ đối tượng này sang đối tượng khác và sinh tự động thông tin tạo mới hoặc cập nhật
        /// </summary>
        /// <typeparam name="TSource"></typeparam>
        /// <typeparam name="TDestination"></typeparam>
        /// <param name="source"></param>
        ...
        public static void MapAudit<TSource, TDestination>(TSource source, TDestination destination, string currentUser)
        {
            ...
            #region Xử lý tạo những thuộc tính sinh tự động
            ...
            // Sinh tự động thông tin tạo mới hoặc cập nhật
            if (!string.IsNullOrEmpty(currentUser))
            {
                if (isNew)
                {
                    destinationProps.FirstOrDefault(p => p.Name == Constants.Id)?.SetValue(destination, Guid.NewGuid());
                    destinationProps.FirstOrDefault(p => p.Name == Constants.UserCreate)?.SetValue(destination, currentUser);
                    destinationProps.FirstOrDefault(p => p.Name == Constants.DateCreate)?.SetValue(destination, DateTime.Now);
                }
                else
                {
                    destinationProps.FirstOrDefault(p => p.Name == Constants.UserUpdate)?.SetValue(destination, currentUser);
        ...
        /// <summary>
        /// Dùng để mapping dữ liệu từ list đối tượng này sang list đối tượng khác và sinh tự động thông tin tạo mới hoặc cập nhật
        /// </summary>
        /// <typeparam name="TSource"></typeparam>
        /// <typeparam name="TDestination"></typeparam>
        /// <param name="sourceList"></param>
        ...
        public static T CopyImport<T>(IRow headerRow, IRow dataRow) where T : new()
        {
            ...
            #region Gán giá trị mặc định cho những thuộc tính hệ thống
